...
<body>
  ...
  <h2 style="font-size:25px;padding:10px 10px 0;justify-content:center;display:flex">Thêm sản phẩm</h2>

    <form action="" method="POST" style="padding: 20px"> 
        Name: <input type="text" name="name" class="form-item"></br>
        Price: <input type="number" name="price" class="form-item"> </br>
        Image: <input type="file" name="img" class="form-item"> </br>
        Image2: <input type="file" name="img2" class="form-item"> </br>
    ...
    <?php 
      ...
      class data
      {
        public function in_product($name,$price,$img,$img2,$des,$dataitem)
          {
            global $conn;
            $sql="insert into dt_detail(Name,Price,Image,Image2,Description,Data_item) values('$name','$price','$img','$img2','$des','$dataitem')";
        ...
        public function select_product($name)
          {
            global $conn;
            $sql="select * from dt_detail where Name='$name'";
            $run=mysqli_query($conn,$sql);
      ...
      if(empty($_POST['name']) || empty($_POST['price']))
      {
        ...
        echo "Sản phẩm đã tồn tại.Hãy chọn sản phẩm khác";
          ...
          $insert=$get_data->in_product($_POST['name'],
            $_POST['price'],
            $_POST['img'],
            $_POST['img2'],
            $_POST['des'],
